...
- ปุ่ม `เปลี่ยนชื่อรูป` ใช้ได้กับรูปทุกแหล่ง ได้แก่ รูปจากกล้องเครื่อง/OPPO รูปนำเข้าจาก Gallery รูป Timestamp และรูปจาก PDF; ปุ่มดินสอเปลี่ยนชื่อใช้กับวิดีโอที่นำเข้าได้ด้วย
- DN พยายามเปลี่ยน `DISPLAY_NAME` ของไฟล์จริงและขออนุญาตผ่านหน้าต่างระบบเมื่อ Android ต้องการ เพื่อให้ชื่อใน Gallery เปลี่ยนตาม หาก Photo Picker ให้ URI แบบอ่านอย่างเดียว DN จะยังเปลี่ยนชื่อที่แสดงในแอปและชื่อสำหรับ Backup โดยไม่สร้างไฟล์ซ้ำ
- หน้า PhotoViewer แบบเต็มจอเพิ่มปุ่มเมนูใต้ไอคอน `i` สำหรับเปลี่ยนชื่อรูปทุกแหล่ง, ย้ายรูปด้วย file-browser เลือกงาน/โฟลเดอร์หลายระดับ และลบรูปพร้อมคำยืนยัน/MediaStore approval โดยไม่ต้องย้อนกลับไปหา thumbnail
- การนำเข้ารูปจาก Gallery แสดง AlertDialog `กำลังนำเข้ารูป` แบบปิดไม่ได้ตลอดช่วงอ่าน EXIF และบันทึก Room พร้อม progress `ไฟล์ปัจจุบัน/ทั้งหมด`; ป้องกันผู้ใช้เปลี่ยนงานหรือย้อนกลับก่อน coroutine นำเข้าเสร็จ
- เมื่อทำงานครบ หน้าต่างไม่หายเอง แต่เปลี่ยนเป็นสรุปจำนวนสำเร็จ/ข้าม/ผิดพลาดและค้างไว้จนผู้ใช้กด `ดูรูปที่นำเข้า` เท่านั้น ปุ่มนี้ปิดเฉพาะ dialog และคงอยู่ในโฟลเดอร์เดิม ไม่ปิดแอปหรือย้อนหน้า; การแตะนอกหน้าต่างหรือกดย้อนกลับจะไม่ปิด
